...
$sub.Text = "Platform 5.0 Milestone 1 * Powered by EMO`r`nThis bootstrap installs the preview, then the guided Platform Builder configures HandBrake, storage, workflow and media server settings."
...
$install.Add_Click({
    ...
    try {
        ...
        [System.Windows.Forms.Application]::DoEvents()
        $launchArgs = $pyArgs + @((Join-Path $installDir 'EvilsMediaOptimizer.pyw'),'--platform-builder')
