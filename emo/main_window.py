...
class LiveTelemetryPanel(QFrame):
    ...
    def nvidia_metrics(self) -> tuple[float, float]:
        try:
            result = subprocess.run(
                [
                    "nvidia-smi",
            ...
            usage, temperature = [
                float(part.strip())
                for part in first_line.split(",")[:2]
            ]
    ...
    def sample(self):
        gpu, temperature = self.nvidia_metrics()
...
class MainWindow(QMainWindow):
    ...
    def update_stats(self):
        self.queue=[m for m in self.movies if m.queued]; self.optimized_card.value.setText(str(len(self.queue))); self.saving_card.value.setText(human_size(sum(m.saving for m in self.queue)))
    ...
    def process_next(self):
        ...
        if not self.queue:
            ...
            return
        movie=self.queue[0]; self.current_movie=movie; movie.status="Working"; movie.queued=False; self.update_stats(); self.populate_table(); self.status.setText(f"Starting {movie.title}..."); self.progress.setValue(0)
    ...
    def show_dashboard(self):
        ...
        QMessageBox.information(
            ...
            f"Queued: {len(queued)}\n"
            f"Potential saving: "
            f"{human_size(sum(movie.saving for movie in queued))}\n"
            f"Library: {self.config.get('movie_root', '')}",
        )
    ...
    def show_settings(self):
        dialog=SettingsDialog(self.config,self)
        if dialog.exec()==QDialog.DialogCode.Accepted:
            self.config=dialog.values(); save_config(self.config); self.found_card.findChild(QLabel,"statSub"); self.status.setText("Settings saved. Run a new scan to apply library changes.")
