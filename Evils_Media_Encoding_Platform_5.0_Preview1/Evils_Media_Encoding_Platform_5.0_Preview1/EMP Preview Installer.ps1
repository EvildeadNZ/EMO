...
$install.Add_Click({
    ...
    try {
        ...
        Start-Process -FilePath $pyExe -ArgumentList $launchArgs -WorkingDirectory $installDir
        [System.Windows.Forms.MessageBox]::Show("EMP is installed. The Platform Builder will now guide you through HandBrake, workflow, locations and media-server setup.", $product, 'OK', 'Information') | Out-Null
        $form.Close()
    } catch {
