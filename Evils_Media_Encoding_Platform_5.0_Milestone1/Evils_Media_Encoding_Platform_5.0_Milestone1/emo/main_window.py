...
from PySide6.QtGui import QColor, QPixmap, QIcon, QPainter, QPainterPath, QPen, QBrush, QDesktopServices
from PySide6.QtWidgets import (
    ...
    QListWidgetItem, QTabWidget, QGroupBox, QAbstractItemView, QWizard, QWizardPage,
...
def version_key(value: str):
    match = re.match(r"^(\d+)\.(\d+)\.(\d+)(?:[-.]?(preview|m|milestone|rc)(\d+)?)?$", value.strip().lower())
    if not match:
    ...
    def run(self):
        ...
        try:
            req = urllib.request.Request(
                ...
                headers={"Accept":"application/vnd.github+json", "User-Agent":"EMP-Updater"},
            )
...
class SettingsDialog(QDialog):
    def __init__(self, config: dict, parent=None):
        ...
        self.github_repo.setPlaceholderText("owner/repository  (example: EvilCore/EMP)")
...
class MainWindow(QMainWindow):
    ...
    def update_theme_banner(self):
        """
        Use the original polished EMO banner for every theme.

        Theme colours still change normally. The experimental generated
        artwork has been disabled because it did not meet the application's
    ...
    def encode_done(self, movie, path):
        ...
        # Keep the queue alive with an owned timer.  Using a persistent timer
        # makes the hand-off between worker jobs deterministic and prevents
        # a completed first item from leaving the remaining queue idle.
        if remaining > 0 and self.queue_running:
            self.status.setText(
                f"Completed: {movie.title} — next queue item starting shortly..."
            )
            self.queue_advance_timer.start(1200)
        else:
            self.process_next()
    ...
    def set_update_light(self, text: str, color: str, tip: str = ""):
        ...
        self.update_button.setStyleSheet(f"color:{color};font-weight:800;")

    def check_for_updates(self):
        repo = str(self.config.get("github_repo", "")).strip()
        if not repo:
            self.set_update_light("●  GIT NOT SET", "#8f8794", "Set owner/repository under Settings > Updates")
            return
        self.set_update_light("●  CHECKING...", "#ffbd59", f"Checking GitHub: {repo}")
    ...
    def on_update_checked(self, result: dict):
        ...
        if not result.get("ok"):
            self.set_update_light("●  UPDATE CHECK FAILED", "#ff6879", result.get("error", "Update check failed"))
        elif result.get("newer"):
            self.set_update_light(f"●  UPDATE {result.get('version','')} AVAILABLE", "#d35cff", "Click to download and install from GitHub")
        else:
            self.set_update_light("●  UP TO DATE", "#70df7b", f"Installed {APP_VERSION} is current")

    def update_button_clicked(self):
        result = self.latest_release or {}
        if result.get("ok") and result.get("newer") and result.get("download_url"):
            answer = QMessageBox.question(self, "EMP update available",
                f"EMP {result.get('version')} is available on GitHub.\n\nDownload, install and restart now?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            if answer == QMessageBox.StandardButton.Yes:
                try:
                    updates_dir = APP_DIR / "_updates"; updates_dir.mkdir(exist_ok=True)
                    target = updates_dir / (result.get("asset_name") or "EMP-update.zip")
                    req = urllib.request.Request(result["download_url"], headers={"User-Agent":"EMP-Updater"})
                    with urllib.request.urlopen(req, timeout=60) as src, target.open("wb") as dst:
                        shutil.copyfileobj(src, dst)
                    new_version = launch_external_update(target, APP_DIR, current_version=APP_VERSION, current_pid=os.getpid())
                    QMessageBox.information(self, "Update ready", f"EMP {new_version} downloaded. EMP will close, update, and reopen.")
                    QTimer.singleShot(250, QApplication.instance().quit)
                except Exception as exc:
                    QMessageBox.critical(self, "Update failed", str(exc))
            return
        self.show_update_center()
    ...
    def show_settings(self):
        ...
        if dialog.exec()==QDialog.DialogCode.Accepted:
            ...
            self.operations.refresh_services()
            self.status.setText(
                f"Settings saved. Theme: "
                f"{self.config.get('theme', 'Skull Purple')}."
            )
...
def main():
    ...
    config = load_config()
    if not bool(config.get("setup_complete", False)):
